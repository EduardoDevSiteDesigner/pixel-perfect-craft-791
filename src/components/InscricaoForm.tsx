...
const InscricaoForm = () => {
  ...
  const [formData, setFormData] = useState({
    nomeCompleto: "", nomeArtistico: "", email: "", pais: "", cep: "",
    estado: "", cidade: "", bairro: "", endereco: "", numero: "",
    complemento: "", cpf: "", celular: "", tituloPoema: "", concordo: false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nomeCompleto.trim() || !formData.email.trim()) {
      toast.error("Preencha pelo menos o nome completo e e-mail.");
      return;
    }
  ...
  const inputClass = "w-full px-4 py-3 border-2 border-navy rounded-md bg-primary-foreground text-navy focus:outline-none focus:ring-2 focus:ring-magenta";

  const fields = [
    { label: "Nome Completo", name: "nomeCompleto" },
    { label: "Nome artístico (como seu nome será impresso no livro)", name: "nomeArtistico" },
    ...
    { label: "Título do poema", name: "tituloPoema" },
  ];
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <label className="border-2 border-dashed border-navy rounded-md p-8 text-center cursor-pointer hover:bg-primary-foreground/50 transition-colors block">
            ...
            <p className="text-navy font-semibold">
              {arquivo ? arquivo.name : "Carregar Arquivo"}
            </p>
          </label>
